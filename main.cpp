...
class AFD
{
    ...
    private:
        ...
        vector<string> EstadosAlcancaveis; // Lista de estados alcançáveis, a partir do estado inicial
        vector<string> In_Estados_Al; // Lista de estados alcançáveis, a partir de um estado
        ...
        void Transicao(char c)
        {
            // Testa se entrada é válida
                ...
                Situacao = -2;
                // Tenta encontrar se o símbolo faz parte do alfabeto
                for(char x: A){
                    if(x == c)
                    {
...
//----------Requisitos------------------------------------------------------------------------------------
            ...
            // Somente Estados Alcançáveis
            ApenasAlcancaveis(i); // Atualiza os Estados Alcançáveis a partir do estado inicial

            // Passa por todos os estado do AFD, se não for Alcançável, o exclui
            for(string Estado: S)
...
//----------Etapas da Minimização-------------------------------------------------------------------------
            // Teste de equivalência de estados-----------------------------------------------------------
                ...
                // Vai por toda a tabela de equivalência e verifica se os pares são ambos do conjuntos dos finais,
                // se um for e o outro não, ele marca com um X
            ...
            for(unsigned int ic = 0; ic<EstadosAlcancaveis.size()-1; ic++)
            {
                for(unsigned int j = 0; j<=ic; j++)
                {
                    ...
                    // Passa por todas as transições, e qualquer transição que seja direcionada a alguma das partes é redirecionada
            ...
            for(string Estado: EstadosAlcancaveis)
            {
                ...
                //  Verificar se algum estado alcançável por ele é final
        ...
        void ApenasAlcancaveis(string Estado)
        {
            ...
            // Repete os mesmo processo para os estados alcançáveis
            for(char a: A)
            {
                try{
        ...
        // Mesmo processo do ApenasAlcancaveis, a única diferença é que guarda os estados alcançáveis no vetor In_Estados_Al
        ...
        // Remove um estado da lista de estados alcançáveis e da lista de transição
        void Remove_Estado(string Estado)
        {
            // Remove o estado das lista de Estados Alcançáveis
            ...
            // Remove as transições que contêm o Estado
        ...
        void VerificaPar(string p, string q)
        {
            ...
            for(char a: A)
            {
                ...
                // Se o par r,s não estiver marcado, adicionar o par p,q como dependentes
                if(TabelaS(r,s)[0] == " ")
                {
        ...
        {
            // Marca p,q e se n�o tiver dependências, para
            TabelaS(p,q)[0] = "X";
